...
// Chrzanów: ONE feed, one operator — Związek Komunalny „Komunikacja Międzygminna” w Chrzanowie,
// the union that runs the buses of Chrzanów, Trzebinia, Libiąż, Alwernia, Babice and Płaza.
// The file comes from kiedyPrzyjedzie, which is where the union publishes:
// no shapes and no direction_id, so the STOP SEQUENCE is the matching
// observation and the headsign is the direction key (the Olsztyn/GPA
// arrangement). Buses only — there is no rail here.
...
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/chrzanow.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['10']),
  feeds: [
    { tag: 'zkm', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['3'] },
  ],
}];
// no tram, no rail on this sheet
const tramAll = false;
...
await (await import('./night.mjs')).nightPass(outDir, /^N$/, { sort: true });
...
await (await import('./railrows.mjs')).railRowPass(outDir, /^N$/, { log });

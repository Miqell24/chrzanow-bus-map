...
# missing.
#
# Chrzanów: ONE feed, one operator — the commune union ZKKM (Związek Komunalny
# „Komunikacja Międzygminna" w Chrzanowie), whose buses serve Chrzanów,
# Trzebinia, Libiąż, Alwernia, Babice and Płaza.
#
# TWO copies of that feed exist and they are NOT the same: the operator's own
# file (zkkm.pl/files/pl/gtfs/google-transit-3.zip) is the kiedyPrzyjedzie
# export and ships NO shapes, while odt.org.pl republishes it WITH shapes
# (32 545 points) — same 26 routes, same 1 636 trips, but the corridors come
# from the publisher instead of being reconstructed from the stop sequence.
# This script takes the ODT copy for that reason. Neither carries direction_id,
# so the headsign is the direction key.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ ! -f data/gtfs/routes.txt ]; then
  ...
  curl -fL --retry 3 --max-time 600 \
    ...
    -o data/gtfs.zip "https://data.odt.org.pl/gtfs/zkkm-gtfs.zip" -d data/gtfs
fi
...
if [ ! -f data/osm/chrzanow.json ]; then
...
echo "OK — data ready:"
du -sh data/gtfs data/osm/chrzanow.json 2>/dev/null || true
